...
from src.methods.theta_method import theta_step

class PdeSolver:
    """
    Solve derivative pricing PDE in Hull White model.
    """
    ...
    def roll_back(self, T0, T1, x1, U1, H1):
        ...
        for k in range(tGrid.shape[0]-1):
            ...
            V = self.roll_back_one_step(tGrid[k+1],tGrid[k],x1,V)
        return (x1, V)

    def roll_back_one_step(self, T0, T1, x, V):
        ...
        f     = self.hwModel.forward_rate(0.0,0.0,t)
        sigma = self.hwModel.sigma(t)
        y     = self.hwModel.y(t)
        a     = self.hwModel.mean_reversion
        ...
        V = theta_step(l[1:], c, u[:-1],V,ht,self.theta)
        return V
